---
name: nanlabs-work-item
description: >-
  WHAT - Router for creating and refining epics, user stories, tasks, bugs, and incidents using NaNLABS Best Practices work item hierarchy.
---
...
1. Apply **`nanlabs-output-handshake`** before producing final content or writing to a tool.
2. Use the smallest applicable skill:
   - Epic -> `nanlabs-epic`
   - User story -> `nanlabs-user-story`
   - Technical task -> `nanlabs-task`
   - Bug -> `nanlabs-bug`
   - Incident -> `nanlabs-incident`
3. Use **`nanlabs-planning`** when the item needs breakdown, estimation, prioritization, or capacity checks.
