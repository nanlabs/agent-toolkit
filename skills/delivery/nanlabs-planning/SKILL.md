---
name: nanlabs-planning
description: >-
  WHAT - Planning, estimation, task breakdown, and iteration capacity fallback based on NaNLABS Best Practices. Use before finalizing backlog scope, story/task estimates, or iteration commitments.
---
...
1. Apply **`nanlabs-output-handshake`** before producing any final planning artifact: ask where it should live and confirm human review.
...
3. If a work item is being created or refined, pair this with **`nanlabs-work-item`** and the relevant atomic skill.

## What to check

- **Blast radius** and rollback are explicit for each planned step (see `nanlabs-planner` agent `references/CONTRACT.md`).
- **User approval** before implementation when the plan is non-trivial.
