...
name: nanlabs-adr
description: >-
  WHAT, Create and maintain Architecture Decision Records (ADRs) per the NaNLABS process in ClickUp. Covers when to write an ADR, required sections, review, and linking to epics/PRs. English for cross-team artifacts unless the user asks otherwise.
---
...
1. Apply **`nanlabs-output-handshake`**: confirm **where** the final ADR will be recorded and that a **human** will review.
2. Then follow the steps below.

## When to use

Use **`nanlabs-decision-log`** for lightweight product/project/operational decisions and **`nanlabs-agreement`** for explicit commitments or terms among parties. Use this ADR skill only for durable architecture or technical decisions.
...
- `nanlabs-decision-log`, lightweight decisions
- `nanlabs-agreement`, explicit commitments or terms
- `nanlabs-workflow-generic-project`, traceability and plan approval
