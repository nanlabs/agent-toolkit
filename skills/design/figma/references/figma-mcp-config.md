...
from an environment variable. NaNLABS ships a tool-agnostic template at
`~/.local/share/nanlabs/mcp/figma/` (deployed by `chezmoi apply`).
...
3. Store it persistently in `~/.config/nanlabs/env.d/` (covered by `nan-loadenv`):

   ```bash
   mkdir -p ~/.config/nanlabs/env.d
   cat >> ~/.config/nanlabs/env.d/figma.env <<'EOF'
   export FIGMA_OAUTH_TOKEN="<paste-token-here>"
   export FIGMA_REGION="us-east-1"   # change if your org uses a different region
   EOF
   chmod 600 ~/.config/nanlabs/env.d/figma.env
   ```

   `nan-doctor` will pick up `figma.env` under "Integrations" without ever
...
`~/.local/share/nanlabs/mcp/figma/config.template.json` as reference:
...
tool. With `nan-loadenv` already wired in `nan-bootstrap`, sourcing your shell
profile is enough.

## 3) Verify

1. Run `nan-doctor` and confirm `~/.config/nanlabs/env.d/figma.env` is listed
   under Integrations.
