...
- `gh` installed and authenticated (`gh auth status` exits 0).
  `nan-doctor` reports this under Integrations.
...
6. **Draft a plan.** Propose the fix as a short, numbered plan. **Do not edit
   files yet.** If the project has a planning skill (e.g. `nanlabs-planning`),
...
- `nan-skills check` reports the `gh` and `python3` requirements.
