...
- `gh` (GitHub CLI) installed and on `PATH` (covered by `nan-bootstrap`).
...
`nan-doctor` already reports `gh auth status` under "Integrations" — confirm it
...
- `nan-skills check` reports the `gh` requirement.
- `nan-doctor` shows `gh auth status` under Integrations.
