...
description: Create, scaffold, or refactor Jupyter notebooks (.ipynb) for experiments and tutorials. Prefer the bundled templates and helper script (`new_notebook.py`, exposed as `nan-newnotebook`) over authoring raw notebook JSON.
---
...
- `python3` (provided by `nan-bootstrap`).
- Optional: `uv` for local notebook execution and dependency isolation
  (also from `nan-bootstrap`).

`nan-skills check jupyter-notebook` validates `python3` is present.
...
### Option A — `nan-newnotebook` wrapper (recommended)

`nan-bootstrap` installs a tiny wrapper at `~/.local/bin/nan-newnotebook` so
the script is available without remembering its full path:

```bash
nan-newnotebook --kind experiment \
  --title "Compare prompt variants" \
  --out output/jupyter-notebook/compare-prompt-variants.ipynb

nan-newnotebook --kind tutorial \
...
2. **Scaffold from the template** (use `nan-newnotebook` or option B above).
