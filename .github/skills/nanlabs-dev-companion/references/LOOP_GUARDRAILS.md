# Autonomous loop guardrails (NaNLABS distilled)
...
After `chezmoi apply`, see `~/.local/share/nanlabs/third-party/everything-claude-code/agents/loop-operator.md` (MIT; **`NOTICE.md`** in that directory). In the **internal-workstation** git checkout, the same files live under `home/dot_local/share/nanlabs/third-party/everything-claude-code/`.
