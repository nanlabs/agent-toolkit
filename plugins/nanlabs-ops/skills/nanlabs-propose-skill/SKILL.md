...
Paths below (`docs/`, `catalogs/`, `plugins/`) are relative to
...
2. **Duplicate check** — In **nanlabs/agent-toolkit**, search
   ...
   `catalogs/agent-catalog.yaml`, and `plugins/nanlabs-<group>/skills/` for the same job. If a
...
3. **Author** — Follow `docs/AUTHORING.md` and `docs/CONTRIBUTION.md` in
   **nanlabs/agent-toolkit**. Layout: `plugins/nanlabs-<group>/skills/<skill>/SKILL.md` with
   kebab-case `name` matching the directory. That path **is** the Agent Plugins
   package skill (`plugins/<id>/skills/<name>/SKILL.md`). Do **not** add a second
   copy under `skills/<group>/`. Do **not** add `plugin.json` for a catalog **domain**
   pack. Register the skill in `catalogs/skills-layout.json`; `gen-surfaces` writes
   manifests only.
4. **Pack membership** — Add the skill to `catalogs/skill-catalog.yaml` and
   ...
   If you changed `plugins/` or the layout, also run
   `python3 scripts/validate-skill-inventory.py` and
