---
name: nanlabs-workflow-client-bootstrap
description: >-
  WHAT — Interactive interview that stores client delivery context as packs + knowledge inside the
  user's ~/.nan-ai-workspace (no client skills). Use when onboarding a new client project or
  updating an existing workspace context.
---

# Workflow Client Bootstrap

Use this skill to **create or update** a client-specific delivery context for any NaNLABS project.
It runs a structured interview and stores the result inside the user’s **workspace**
(`~/.nan-ai-workspace`, `~/.ai-workspace`, or a workspace-like directory) as **packs + knowledge**
(not new skills).
...
- Onboarding a new client project that needs its own delivery workflow (beyond `nanlabs-workflow-generic-project`)
...
- The work fits entirely within `nanlabs-workflow-generic-project` without client-specific gates
...
| Repo discovery | **nanlabs-assistant** |
